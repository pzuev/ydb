...
private:
    ...
    TUnboxedValueVector StateBuffer;
...
public:
    TGenericAggregation(
        ...
        , Nodes(nodes)
        , Layout(layout)
        , StateBuffer(Nodes.StateNodes.size())
    {
    }
    ...
    void UpdateState(void* rawState, TUnboxedValue* const* /*row*/) override final {
        Layout.UnpackMove(rawState, StateBuffer);
        for (size_t i = 0; i < Nodes.StateNodes.size(); ++i) {
            Nodes.StateNodes[i]->SetValue(Ctx, std::move(StateBuffer[i]));
        }
        for (size_t i = 0; i < Nodes.UpdateResultNodes.size(); ++i) {
            StateBuffer[i] = Nodes.UpdateResultNodes[i]->GetValue(Ctx);
        }
        Layout.PackMove(StateBuffer, rawState);
    }
    ...
    void InitState(void* rawState, TUnboxedValue* const* /*row*/) override final {
        for (size_t i = 0; i < Nodes.InitResultNodes.size(); ++i) {
            StateBuffer[i] = Nodes.InitResultNodes[i]->GetValue(Ctx);
        }
        Layout.PackMove(StateBuffer, rawState);
    }
    ...
    void ExtractState(void* rawState, TUnboxedValue* const* output) override {
        Layout.UnpackMove(rawState, StateBuffer);
        for (size_t i = 0; i < Nodes.FinishStateNodes.size(); ++i) {
            Nodes.FinishStateNodes[i]->SetValue(Ctx, std::move(StateBuffer[i]));
        }
...
protected:
    ...
    TCoroTask ReadBackNextSpillingBucketAsync()
    {
...

        {
            ...
            while (!inputSpiller.Empty()) {
                ...
                char* record = nullptr;
                bool isNew = false;
                RecordLayout.GetKeyLayout().PackBorrowed(TempKeyBuffer, PackedKeyBuffer.data());
                char* mapIt = Map->Insert(PackedKeyBuffer.data(), GlobalHashToRhItemHash(Hasher(TempKeyBuffer.data())), isNew);
                char* statePtr = nullptr;

                if (isNew) {
                    record = static_cast<char*>(Store->Alloc(0));
                    RecordLayout.Clear(record);
                    RecordLayout.GetKeyLayout().CopyWithRefs(PackedKeyBuffer.data(), record);
    ...
    EFillState ProcessFetchedRow(TUnboxedValue* const* input, TArrayRef<TUnboxedValuePod> keyBuf, ui64 hash) {
        ...
        RecordLayout.GetKeyLayout().PackBorrowed(keyBuf, PackedKeyBuffer.data());
        char* record = nullptr;
        bool isNew = false;
        auto mapIt = Map->Insert(PackedKeyBuffer.data(), GlobalHashToRhItemHash(hash), isNew);
        char* statePtr = nullptr;
        if (isNew) {
...
public:
        ...
        , TestParams(testParams)
    {
        ...
        PackedKeyBuffer.resize(std::max<size_t>(RecordLayout.GetKeyLayout().GetSize(), 1));
        DrainKeyBuffer.resize(KeyTypes.size());
        ...
        if (HasGenericAggregation) {
            auto genericAgg = std::make_unique<TGenericAggregation>(Ctx, Nodes, RecordLayout.GetStateLayout());
...
protected:
    ...
    std::vector<char> PackedKeyBuffer;
    TUnboxedValueVector DrainKeyBuffer;
...
public:
    ...
    NUdf::EFetchStatus TryDrainInternal(NUdf::TUnboxedValue* const* outputPtrs) {
        ...
        if (HasGenericAggregation) {
            RecordLayout.GetKeyLayout().UnpackMove(tuple, DrainKeyBuffer);
            for (ui32 i = 0U; i < Nodes.FinishKeyNodes.size(); ++i) {
                Nodes.FinishKeyNodes[i]->SetValue(Ctx, std::move(DrainKeyBuffer[i]));
            }
        }
